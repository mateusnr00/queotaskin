...
| ENCRYPTION | PAYMENT_SECRET_ENCRYPTION_KEY | cifra secrets de gateway (write-only) | server | sim | especial (ver rotação) | fail-fast |
| ENCRYPTION | ADMIN_MFA_ENCRYPTION_KEY | cifra segredos TOTP do admin (v2, domínio separado) | server | sim | especial (recipher legado) | fail-fast; **deve ≠ PAYMENT_SECRET_ENCRYPTION_KEY** |
...
| ~~OTP~~ | (nenhuma) | ~~provider de entrega participante~~ — **removido na FASE 10.2** (CPF+senha); nenhum OTP externo requerido | — | não | — | — |
